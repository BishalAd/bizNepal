...
### Workflow 9: Telegram Chatbot Signup (State Machine) - BUGFIXED

This workflow is perfectly state-managed. It intercepts both messages and button clicks to keep track of the user's progress. Instead of losing data between nodes, it forcefully passes the exact `userId` downstream so your chatbot cannot get "amnesia" or loop at the welcome screen.
...
```json
{
  ...
  "nodes": [
    ...
    },
    {
      "parameters": {
        ...
        "sendBody": true,
        "specifyBody": "json",
        "jsonBody": "={\n  \"id\": \"{{$json.user.id}}\",\n  \"full_name\": \"{{$('Session Manager').item.json.sessionData.fullname}}\",\n  \"role\": \"{{$('Session Manager').item.json.sessionData.role}}\",\n  \"phone\": \"{{$('Session Manager').item.json.sessionData.phone}}\"\n}",
        "options": {}
      },
    ...
    {
      "parameters": {
        "conditions": { "string": [ { "value1": "={{ $('Session Manager').item.json.sessionData.role }}", "value2": "business" } ] }
      },
    ...
    },
    {
      "parameters": {
        ...
        "sendBody": true,
        "specifyBody": "json",
        "jsonBody": "={\n  \"name\": \"{{$('Session Manager').item.json.sessionData.bizName}}\",\n  \"owner_id\": \"{{$('Supabase SignUp Auth').item.json.user.id}}\"\n}",
        "options": {}
      },
    ...
    {
      "parameters": {
        "jsCode": "const staticData = $getWorkflowStaticData('global');\ndelete staticData.sessions[$('Session Manager').item.json.userId];\nreturn $input.all();"
      },
  ...
  "connections": {
    ...
    "Send Welcome": { "main": [ [ { "node": "Set ask_manual_or_chat", "type": "main", "index": 0 } ] ] },
    "Manual or Chat?": { "main": [ [ { "node": "Send Manual Link", "type": "main", "index": 0 } ], [ { "node": "Ask Name", "type": "main", "index": 0 } ] ] },
    "Send Manual Link": { "main": [ [ { "node": "Clear Session (Manual)", "type": "main", "index": 0 } ] ] },
    "Ask Name": { "main": [ [ { "node": "Set ask_name", "type": "main", "index": 0 } ] ] },
    ...
    "Supabase SignUp Auth": { "main": [ [ { "node": "Supabase Insert Profile", "type": "main", "index": 0 } ] ] },
    "Supabase Insert Profile": { "main": [ [ { "node": "Check Business Submit", "type": "main", "index": 0 } ] ] },
